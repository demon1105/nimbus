...
- (void)exchangeObjectFromIndex:(NSUInteger)fromIndex toIndex:(NSUInteger)index inSection:(NSUInteger)sectionIndex
{
    NIDASSERT(sectionIndex >= 0 && sectionIndex < self.sections.count);
    NITableViewModelSection *section = [self.sections objectAtIndex:sectionIndex];
    [section.mutableRows exchangeObjectAtIndex:index
                             withObjectAtIndex:fromIndex];
}
